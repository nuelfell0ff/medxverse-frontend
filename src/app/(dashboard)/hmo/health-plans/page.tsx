...
const API_BASE = RAW_API.endsWith("/api/v1")
  ? RAW_API
  : `${RAW_API}/api/v1`;

const PLANS_API = `${API_BASE}/health-plans`;
const BENEFITS_API = `${API_BASE}/benefits`;
...
function toForm(plan: HealthPlan): PlanForm {
  return {
    ...
    individualPremium:
      plan.premium?.individual === undefined
        ? ""
        : String(plan.premium.individual),
    familyPremium:
      plan.premium?.family === undefined
        ? ""
        : String(plan.premium.family),
    corporatePremium:
      plan.premium?.corporate === undefined
        ? ""
        : String(plan.premium.corporate),
    defaultWaitingPeriodDays: String(
      plan.defaultWaitingPeriodDays ?? 0,
    ),
    annualUtilizationLimit:
      plan.annualUtilizationLimit === undefined
        ? ""
        : String(plan.annualUtilizationLimit),
    effectiveFrom: toInputDate(plan.effectiveFrom),
    effectiveTo: toInputDate(plan.effectiveTo),
    notes: plan.notes || "",
    benefitIds: Array.isArray(plan.benefitIds)
      ? plan.benefitIds.map((benefit) =>
          typeof benefit === "string" ? benefit : benefit._id,
        )
      : [],
  };
}
...
function buildPayload(form: PlanForm) {
  ...
  if (form.individualPremium !== "") {
    premium.individual = Number(form.individualPremium);
  }

  if (form.familyPremium !== "") {
    premium.family = Number(form.familyPremium);
  }

  if (form.corporatePremium !== "") {
    premium.corporate = Number(form.corporatePremium);
  }
  ...
  return {
    ...
    defaultWaitingPeriodDays: Number(
      form.defaultWaitingPeriodDays || 0,
    ),

    ...(form.annualUtilizationLimit !== ""
      ? {
          annualUtilizationLimit: Number(
            form.annualUtilizationLimit,
          ),
    ...
    // IMPORTANT:
    // This is what was missing from the original version.
  ...
  return (
    <label className="block">
      ...
      <select
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs font-medium text-slate-700 outline-none focus:border-[#1b7b68] focus:ring-2 focus:ring-[#1b7b68]/10"
      >
        {options.map((option) => (
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-slate-950/40 p-3 backdrop-blur-[2px] sm:p-5">
      <div className="flex min-h-full items-center justify-center">
        <div
          className={`flex max-h-[calc(100vh-1.5rem)] w-full flex-col overflow-hidden rounded-[28px] bg-white shadow-2xl sm:max-h-[calc(100vh-2.5rem)] ${
...
function BenefitPicker({
  ...
  const filteredBenefits = useMemo(() => {
    ...
    return benefits.filter((benefit) => {
      return (
        ...
        benefit.code.toLowerCase().includes(term) ||
  ...
  return (
    <section className="rounded-3xl border border-[#cfe8e2] bg-[#f4fbf9] p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[10px] leading-4 text-slate-400">
            Select the benefits that belong to this health plan.
          </p>
        </div>

        <span className="rounded-full bg-[#1b7b68] px-2.5 py-1 text-[10px] font-extrabold text-white">
          {selectedIds.length} selected
        </span>
      </div>

      {error && (
        <div className="mt-4 flex items-start gap-2 rounded-2xl border border-rose-200 bg-rose-50 p-3 text-[11px] font-medium text-rose-700">
      ...
      <div className="relative mt-4">
        ...
        <input
          ...
          placeholder="Search benefits by name, code or category..."
          className="w-full rounded-2xl border border-slate-200 bg-white py-3 pl-10 pr-4 text-xs font-medium outline-none focus:border-[#1b7b68] focus:ring-2 focus:ring-[#1b7b68]/10"
        />
      </div>
        ...
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="mt-1 text-[10px] text-slate-400">
              Create an active benefit in the Benefits module first.
            </p>
          </div>
        ) : (
            ...
            {filteredBenefits.map((benefit) => {
              ...
              return (
                <button
                  key={benefit._id}
                  type="button"
                  onClick={() => toggleBenefit(benefit._id)}
          ...
          {selectedIds.map((id) => {
            ...
            return (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full bg-[#1b7b68] px-2.5 py-1.5 text-[10px] font-bold text-white"
              >
                {benefit.name}
...
export default function HealthPlansPage() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${PLANS_API}/stats`, {
        headers: authHeaders(),
        credentials: "include",
        cache: "no-store",
      });
      ...
      setStatsError(
        err instanceof Error
          ? err.message
          : "Unable to load plan statistics",
      );
    }
  }, []);
  ...
  const fetchBenefits = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${BENEFITS_API}?page=1&limit=100&status=ACTIVE`,
        {
          headers: authHeaders(),
          credentials: "include",
          cache: "no-store",
      ...
      const result =
        await parseResponse<BenefitsResponse>(response);

      const received = Array.isArray(result?.benefits)
        ? result.benefits
        : [];

      setBenefits(received);
    } catch (err) {
      setBenefits([]);
      setBenefitsError(
        err instanceof Error
  ...
  const fetchPlans = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        const result =
          await parseResponse<PaginatedPlans>(response);
  ...
  async function openDetails(plan: HealthPlan) {
    ...
    try {
      const response = await fetch(
        `${PLANS_API}/${encodeURIComponent(plan._id)}`,
        {
          headers: authHeaders(),
          credentials: "include",
          cache: "no-store",
      ...
      // Keep already loaded plan as fallback.
    } finally {
  ...
  function validateForm() {
    ...
    const numbers: Array<[string, string]> = [
      ...
      [
        "Waiting period",
        form.defaultWaitingPeriodDays,
      ],
      [
        "Annual utilization limit",
        form.annualUtilizationLimit,
      ],
    ];
  ...
  return (
    <div className="min-h-full space-y-6 font-sans text-slate-800">
      ...
      <section className="flex flex-col gap-4 rounded-3xl border border-slate-100 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs font-medium leading-5 text-slate-400">
            Create and manage insurance plans, premiums,
            coverage periods, waiting periods, utilization
            limits and attached benefits.
          </p>
        </div>
        ...
        {statCards.map((card) => (
          ...
          >
            <div className="flex items-center justify-between gap-2">
              <span
                className={`rounded-xl p-2 ${card.cls}`}
              >
                {card.icon}
              </span>
            ...
            <p className="mt-3 text-[10px] font-extrabold uppercase tracking-wider text-slate-400">
              {card.label}
            </p>
          </div>
        ))}
      ...
      <section className="rounded-3xl border border-slate-100 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-5 xl:flex-row xl:items-center">
          <div className="relative min-w-0 flex-1">
            ...
            <input
              ...
              placeholder="Search plan code, name or description..."
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <input
              ...
              placeholder="Tier"
              className="w-28 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-medium outline-none focus:border-[#1b7b68]"
            />
            ...
            <input
              ...
              className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-medium text-slate-600 outline-none focus:border-[#1b7b68]"
            />

            <button
              type="button"
              onClick={clearFilters}
              className="rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold text-slate-500 hover:bg-slate-50"
            >
              Clear
            </button>
          </div>
              ...
              {loading ? (
                ...
                <tr>
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={openCreate}
                      className="mt-4 rounded-2xl bg-[#1b7b68] px-4 py-2.5 text-xs font-extrabold text-white"
                    >
                      Create Health Plan
                    </button>
                  </td>
                ...
                plans.map((plan) => {
                  ...
                  return (
                    <tr
                      key={plan._id}
                      className="border-b border-slate-50 transition hover:bg-[#e8f5f3]/20"
                    >
                      <td className="px-6 py-4">
                        <button
                          type="button"
                          onClick={() =>
                            void openDetails(plan)
                          }
                          className="text-left"
                        >
                          <div className="font-bold text-slate-800 hover:text-[#1b7b68]">
                            {plan.name}
                          </div>
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[10px] font-bold ${meta.cls}`}
                        >
                          {meta.icon}
                          {meta.label}
                        </span>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-1.5">
                          <button
                            ...
                            className="rounded-xl border border-slate-200 bg-white p-2 text-slate-500 hover:bg-[#e8f5f3] hover:text-[#1b7b68]"
                            title="View"
                          >
                          ...
                          {plan.status !==
                            ...
                            <select
                              ...
                              className="rounded-xl border border-slate-200 bg-white px-2 py-2 text-[10px] font-bold text-slate-500 outline-none"
                            >
                              <option value="">
                                {statusUpdatingId ===
                                plan._id
        ...
        <div className="flex flex-col gap-3 border-t border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-[11px] font-medium text-slate-400">
            Showing {plans.length} of {total} health
            plans
          </p>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-xl bg-slate-50 px-3 py-2 text-[11px] font-bold text-slate-600">
      ...
      <Modal
        ...
        subtitle="Configure plan identity, premiums, coverage rules, lifecycle dates and benefits."
        onClose={() =>
      ...
      >
        <form onSubmit={submitForm} className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-6 overscroll-contain">
            {formError && (
            ...
            <section className="rounded-3xl border border-emerald-100 bg-emerald-50/30 p-5">
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-4">
                ...
                <Field
                  ...
                  type="number"
                  value={form.individualPremium}
                  onChange={(value) =>
                    setForm((f) => ({
                      ...f,
                      individualPremium: value,
                    }))
                  }
                  placeholder="0.00"
                />
                ...
                <Field
                  ...
                  type="number"
                  value={form.corporatePremium}
                  onChange={(value) =>
                    setForm((f) => ({
                      ...f,
                      corporatePremium: value,
                    }))
                  }
                  placeholder="0.00"
                />
            ...
            <section className="rounded-3xl border border-slate-100 bg-slate-50/50 p-5">
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-4">
                ...
                <Field
                  label="Annual Utilization Limit"
                  type="number"
                  value={
                    form.annualUtilizationLimit
                  }
          ...
          <div className="sticky bottom-0 z-10 flex shrink-0 justify-end gap-2 border-t border-slate-100 bg-white/95 px-6 py-4 backdrop-blur-sm">
            <button
              type="button"
              disabled={saving}
              onClick={() => setShowForm(false)}
              className="rounded-2xl border border-slate-200 px-4 py-2.5 text-xs font-bold text-slate-600"
            >
              Cancel
            </button>
            ...
            <button
              type="submit"
              disabled={saving}
              className="inline-flex items-center gap-2 rounded-2xl bg-[#1b7b68] px-5 py-2.5 text-xs font-extrabold text-white disabled:opacity-60"
            >
              {saving && (
      ...
      >
        {selectedPlan && (
          <div className="min-h-0 flex-1 overflow-y-auto p-6 overscroll-contain">
            {detailsLoading && (
              <div className="mb-4 flex items-center gap-2 rounded-2xl bg-slate-50 p-3 text-xs text-slate-500">
                <Loader2 className="h-4 w-4 animate-spin text-[#1b7b68]" />
                Loading latest plan details...
              </div>
            )}
              ...
              <section className="rounded-3xl border border-slate-100 bg-slate-50/50 p-5 lg:col-span-2">
                ...
                <div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs font-bold text-slate-700">
                      {
                        selectedPlan.defaultWaitingPeriodDays
                      }{" "}
                      days
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs font-bold text-slate-700">
                      {selectedPlan.annualUtilizationLimit?.toLocaleString() ||
                        "Unlimited"}
                    </p>
                  </div>
                </div>
                ...
                {selectedPlan.description && (
                  <p className="mt-5 rounded-2xl bg-white p-4 text-xs leading-5 text-slate-500">
                    {selectedPlan.description}
                  </p>
                )}
              </section>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
              <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
                <div className="flex items-center gap-2">
                  <CreditCard className="h-4 w-4 text-[#1b7b68]" />
              ...
              <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
                <div className="flex items-center gap-2">
                  <CalendarDays className="h-4 w-4 text-[#1b7b68]" />
            ...
            <section className="mt-4 rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 grid grid-cols-1 gap-2 md:grid-cols-2">
                {selectedPlan.benefitIds?.length ? (
                  selectedPlan.benefitIds.map(
                    (benefit) => (
                      <div
                        key={benefit._id}
                      ...
                      >
                        ...
                        <span className="rounded-full bg-[#e8f5f3] px-2 py-1 text-[9px] font-extrabold text-[#1b7b68]">
                          {benefit.status ||
                            "DEFINED"}
                        </span>
                      </div>
                  ...
                  <p className="col-span-full py-5 text-center text-xs text-slate-400">
                    No benefits are attached to this
                    plan yet.
                  </p>
                )}
              </div>
            ...
            {selectedPlan.notes && (
              <section className="mt-4 rounded-3xl border border-slate-100 bg-slate-50/60 p-5">
                <h3 className="text-sm font-extrabold text-slate-800">
                  Notes
                </h3>
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-2">
              ...
              {selectedPlan.status ===
                ...
                <button
                  ...
                  className="rounded-2xl bg-emerald-600 px-4 py-2.5 text-xs font-extrabold text-white"
                >
                  Activate
                </button>
              )}
              ...
              {selectedPlan.status ===
                ...
                <button
                  ...
                  className="rounded-2xl bg-orange-500 px-4 py-2.5 text-xs font-extrabold text-white"
                >
                  Deactivate
                </button>
              )}
              ...
              {selectedPlan.status !==
                ...
                <button
                  ...
                  className="rounded-2xl bg-rose-600 px-4 py-2.5 text-xs font-extrabold text-white"
                >
                  Archive
                </button>
              )}
