    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/40 p-0 backdrop-blur-sm sm:items-center sm:p-6">
      <div className="max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-t-[2rem] bg-white shadow-2xl sm:rounded-3xl">
        ...
        <div className="space-y-5 p-6">
          {claim.duplicateRisk && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-amber-600">Duplicate Review Flag</p><p className="mt-1 text-xs font-medium text-amber-800">A similar claim was detected for this member, provider, treatment date and amount. Review before payment.</p></div>}
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
            ...
            <Info label="ICD Code" value={claim.icdCode || '—'} /><Info label="Pre-Authorization" value={claim.preAuthorizationId || 'Not linked'} />
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
            ...
            <div className="rounded-2xl border border-amber-100 bg-amber-50/40 p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-amber-600">Adjusted</p><p className="mt-1 text-lg font-black text-amber-700">{money(claim.adjustedAmount || 0)}</p></div>
            <div className="rounded-2xl border border-sky-100 bg-sky-50/40 p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-sky-600">Payable</p><p className="mt-1 text-lg font-black text-sky-700">{claim.payableAmount === undefined ? 'Pending' : money(claim.payableAmount)}</p></div>
          </div>
          ...
          {claim.notes && <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Notes</p><p className="mt-1 text-xs font-medium leading-5 text-slate-600">{claim.notes}</p></div>}{claim.appeals?.length ? <div className="rounded-2xl border border-slate-100 bg-white p-4"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Appeal History</p><div className="mt-3 space-y-2">{claim.appeals.map((appeal, index) => <div key={index} className="rounded-xl bg-slate-50 p-3"><div className="flex items-center justify-between gap-3"><span className="text-[10px] font-bold uppercase text-slate-500">{appeal.status}</span><span className="text-[10px] text-slate-400">{dateOnly(appeal.submittedAt)}</span></div><p className="mt-1 text-xs text-slate-600">{appeal.reason}</p></div>)}</div></div> : null}
        </div>
