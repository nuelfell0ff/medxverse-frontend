...
interface Claim {
  ...
  /**
   * Optional adjudication / fraud / authorization fields.
   * These may not be present on every API response.
   */
...
export default function ClaimsPage() {
  ...
  const fetchClaims = useCallback(
    async (
      ...
      try {
        ...
        const result = await api<ClaimsResponse>(
          `${CLAIMS_API}?${params.toString()}`,
        );
  ...
  useEffect(() => {
    ...
    // Initial page fetch only.
    // Filter changes are handled above.
  ...
  const stats = useMemo(() => {
    return {
      ...
      claimed: claims.reduce(
        (sum, c) =>
          sum +
          Number(c.totalClaimedAmount || 0),
        0,
      ),

      approvedAmount: claims.reduce(
        (sum, c) =>
          sum +
          Number(c.totalApprovedAmount || 0),
        0,
      ),
    };
  }, [claims, total]);
  ...
  const openClaim = async (claim: Claim) => {
    ...
    try {
      const fresh = await api<Claim>(
        `${CLAIMS_API}/${claim._id}`,
      );
  ...
  return (
    <div className="min-h-full space-y-6 font-sans text-slate-800">
      ...
      <div className="flex flex-col justify-between gap-4 rounded-3xl border border-slate-100 bg-white p-6 shadow-sm sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-xs font-medium text-slate-400">
            Review submitted claims, assess payable
            amounts, and record adjudication decisions.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
            Approved Amount · Current Page
          </p>

          <p className="mt-2 text-2xl font-black text-[#1b7b68]">
            {money(stats.approvedAmount)}
          </p>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                claims.map((claim) => (
                  <tr
                    key={claim._id}
                    className="group transition-all hover:bg-[#e8f5f3]/20"
                  >
                    <td className="px-6 py-4">
                      <div className="font-bold text-sm text-slate-800 group-hover:text-[#1b7b68]">
                        {claim.claimNumber}
                      </div>
                    ...
                    <td className="px-5 py-4">
                      <div className="text-xs font-bold text-slate-700">
                        {entityName(claim.memberId)}
                      </div>

                      {typeof claim.memberId !==
                        'string' &&
                        claim.memberId?.policyNumber && (
        ...
        <div className="flex flex-col gap-4 border-t border-slate-100 bg-slate-50/40 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <p className="text-xs font-medium text-slate-400">
            ...
            · {total.toLocaleString()} total claims
          </p>
...
function StatCard({
  ...
  return (
    <div className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
      <div
        className={`mb-4 flex h-10 w-10 items-center justify-center rounded-2xl ${tones[tone]}`}
      >
        {icon}
      </div>
        ...
        <div className="space-y-5 p-6">
          {claim.duplicateRisk && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
              ...
              <p className="mt-1 text-xs font-medium text-amber-800">
                A similar claim was detected for
                this member, provider, treatment
                date and amount. Review before
                payment.
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
            ...
            <div className="rounded-2xl border border-emerald-100 bg-emerald-50/40 p-4">
              ...
              <p className="mt-1 text-lg font-black text-[#1b7b68]">
                {claim.totalApprovedAmount ===
                undefined
                  ? 'Pending'
                  : money(
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-100">
            <div className="border-b border-slate-100 bg-slate-50/60 px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">
              Claim Items
            </div>
                  ...
                  {claim.items.map(
                    (item, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-bold text-[#1b7b68]">
                          {item.approvedAmount ===
                          undefined
                            ? '—'
                            : money(
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
          {[
            ...
            <button
              ...
              className="rounded-xl bg-[#1b7b68] px-4 py-2.5 text-xs font-bold text-white hover:bg-[#145f50]"
            >
              Open Adjudication
            </button>
          )}
...
function AdjudicationModal({
  ...
  const submit = async () => {
    ...
    if (
      ...
      setError(
        'Select a valid adjudication decision.',
      );
      return;
    }
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-950/40 p-0 backdrop-blur-sm sm:items-center sm:p-6">
      <div className="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-t-[2rem] bg-white shadow-2xl sm:rounded-3xl">
        <div className="flex items-start justify-between border-b border-slate-100 px-6 py-5">
          <div>
            <h2 className="text-lg font-black text-slate-900">
              Adjudicate {claim.claimNumber}
            </h2>

            <p className="mt-1 text-xs text-slate-400">
              Choose the next valid claim status
              and, when approving, set payable
              amounts per item.
            </p>
          </div>
          ...
          {decision === 'APPROVED' && (
            <div className="overflow-hidden rounded-2xl border border-slate-100">
              <div className="border-b border-slate-100 bg-slate-50/60 px-4 py-3">
                ...
                <p className="mt-1 text-[10px] text-slate-400">
                  Set each payable amount.
                  Values are capped at the
                  claimed amount.
                </p>
              </div>
              ...
              <div className="divide-y divide-slate-100">
                {claim.items.map(
                  (item, index) => (
                    <div
                      key={`${item.description}-${index}`}
                      className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-[1fr_180px] sm:items-center"
                    >
                      <div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
          ...
          <button
            ...
            onClick={() => void submit()}
            disabled={saving}
            className="flex items-center gap-2 rounded-xl bg-[#1b7b68] px-4 py-2.5 text-xs font-bold text-white hover:bg-[#145f50] disabled:opacity-50"
          >
            {saving && (
...
function CreateClaimModal({
  ...
  const [preAuthorizationId, setPreAuthorizationId] =
    useState('');

  const [diagnosis, setDiagnosis] =
    useState('');

  const [icdCode, setIcdCode] =
    useState('');
  ...
  const submit = async () => {
    ...
    if (
      ...
      setError(
        'Claim number, member ID, provider ID, diagnosis, and treatment date are required.',
      );
      return;
    }
    ...
    if (
      ...
      setError(
        'Every claim item needs a description, positive quantity, and valid unit price.',
      );
      return;
    }
      ...
      <div className="max-h-[94vh] w-full max-w-4xl overflow-y-auto rounded-t-[2rem] bg-white shadow-2xl sm:rounded-3xl">
        <div className="flex items-start justify-between border-b border-slate-100 px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">
              Create a claim using the exact member
              and provider IDs known to the HMO
              backend.
            </p>
          </div>
        ...
        <div className="space-y-5 p-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <Field
              ...
              placeholder="MongoDB member ObjectId"
            />
            ...
            <Field
              ...
              placeholder="MongoDB provider ObjectId"
            />
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-100">
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/60 px-4 py-3">
              <div>
                ...
                <p className="mt-1 text-[10px] text-slate-400">
                  The backend calculates claimed
                  totals from quantity × unit price.
                </p>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-1.5 rounded-xl bg-[#e8f5f3] px-3 py-2 text-[10px] font-bold text-[#1b7b68]"
              >
            ...
            <div className="divide-y divide-slate-100">
              {items.map(
                (item, index) => (
                  ...
                  >
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_180px_120px_120px_40px]">
                      <div>
                      ...
                      <button
                        ...
                        className="mt-6 flex h-10 w-10 items-center justify-center rounded-xl text-slate-400 hover:bg-rose-50 hover:text-rose-600 disabled:opacity-30"
                      >
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={inputClass}
                        placeholder="Optional service/drug code"
                      />
                    </div>
                  </div>
                ),
            ...
            <div className="flex justify-end border-t border-slate-100 bg-slate-50/40 px-4 py-3">
              <span className="text-xs font-bold text-slate-700">
                Estimated total:{' '}
                <strong className="text-[#1b7b68]">
                  {money(total)}
                </strong>
              </span>
            </div>
